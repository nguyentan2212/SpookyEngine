...
bool SpookyWindow::Initialize(HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height)
{
    ...
    this->handle = CreateWindowEx(0, //Extended Windows style - we are using the default. For other options, see: https://msdn.microsoft.com/en-us/library/windows/desktop/ff700543(v=vs.85).aspx
        ...
        pApplication.get()); //Param to create window
...
bool SpookyWindow::ProcessMessages()
{
...
HWND SpookyWindow::GetHWND() const
{
    return this->handle;
}
...
{
    shared_ptr<SpookyApp> pApplication = SpookyApp::GetInstance();
    return pApplication->WindowProc(hwnd, uMsg, wParam, lParam);
}

void SpookyWindow::RegisterWindowClass()
{
